...
subdir_name = f'data___mp3___{channel_name}___{channel_id}'
...
def get_mp3_by_videoid(yt_id: str, fname_out: str):
    ...
    try:
        ...
        outfile_orig_name = video.download(output_path=subdir_name)
        
        # print(f'outfile_full is type: {type(outfile_full)}')
        # outfile_full_path = Path(outfile_full)
        # outfile_name = outfile_full_path.name
        
        outfile_new_name = f"{os.getcwd()}/{subdir_name}/{fname_out}.mp3"

        # base, ext = os.path.splitext(out_file)
        # filename_out = base + '.mp3'
        print(f'Renaming {outfile_orig_name} to {outfile_new_name}')
        
        # file_src = f"./{subdir_name}/{out_file}"
        # file_dest = f"./{subdir_name}/{fname_out}"
        # print(f'Moving {file_src} to {file_dest}')
        
        Path(outfile_orig_name).rename(outfile_new_name)
        # os.rename(f"./{subdir_name}/{out_file}", f"./{subdir_name}/{fname_out}")    
        
        return out_file # fname_out  # filename_out
